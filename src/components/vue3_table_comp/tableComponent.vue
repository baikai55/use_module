<template>
  <div class="table-content" ref="content">
    <el-table :data="tableData" empty-text="暂无数据~" border ref="table" :height="tableHeightComputed" @select="select"
      @select-all="selectAll" @row-click="rowClick" :tree-props="treeProps" row-key="id">
      <template v-for="(item, index) in header">
        ...
        <el-table-column v-else-if="item.status" :prop="item.prop" :label="item.label" :min-width="item.minWidth"
          ...
          <!-- 状态渲染 0-n -->
          <template #slot="scope">
            <div :style="`color:${changeDisabled(scope.row[item.prop], item.bgc)};`">{{
              scope.row[item.prop] || changeStatus(scope.row[item.prop], item.filters)
            }}</div>
          </template>
        </el-table-column>
              ...
              <template v-for="(btn, ind) in item.tableOption">
                <template v-if="btn.title">
                  <el-popconfirm
                    ...
                    <template #reference>
                      <el-button :type="btn.type" :size="btn.size"
